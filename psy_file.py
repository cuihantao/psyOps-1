...
def insert_header(filename, header):
    '''
    filename: complete path and filename of the text file
    header: string to insert at the top of the text file
    '''
    f = open(filename,'r+')
    lines = f.readlines() # read file content
    f.seek(0) # go back to the beginning of the file
    f.write(header) # write generic csv header line
    for line in lines: # write content after header line
        f.write(line)
    f.close()
